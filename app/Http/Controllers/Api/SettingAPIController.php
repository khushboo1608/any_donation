...
class SettingAPIController extends BaseAPIController
{
    ...
    public function ServiceNeedList(Request $request)
    {    
        try
        {
            $input = $request->all();            
                                            
            $data_service_needs= ServiceNeeds::where('service_needs_status',0)->get();
            $result = $this->GetServiceNeedListResponse($data_service_needs);
            return $this->sendResponse($result, __('messages.api.service.service_get_success'));          
        }
        catch(\Exception $e)
        {
            // $auth_user = Auth::guard('api')->user();
            $this->serviceLogError($service_name = 'ServiceList',$user_id = 0,$message = $e->getMessage(),$requested_field = json_encode($request->all()),$response_data=$e);
            return $this->sendError($e->getMessage(), config('global.null_object'),401,false);
        }
    }
    
    public function SpecificNeedList(Request $request)
    {    
        try
        {
            $input = $request->all();            
                                            
            $data_specific_needs = SpecificNeeds::where('specific_needs_status',0)->get();
            $result = $this->GetSpecificNeedListResponse($data_specific_needs);
            return $this->sendResponse($result, __('messages.api.specific.specific_get_success'));          
        }
        catch(\Exception $e)
        {
            // $auth_user = Auth::guard('api')->user();
            $this->serviceLogError($service_name = 'SpecificNeedList',$user_id = 0,$message = $e->getMessage(),$requested_field = json_encode($request->all()),$response_data=$e);
            return $this->sendError($e->getMessage(), config('global.null_object'),401,false);
        }
    }
}
