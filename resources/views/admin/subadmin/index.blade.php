          ...
          <div class="col-12">
            <div class="card">
                ...
                <div class="col-md-12 col-xs-12">
                    <div class="search_list">
                        <div class="add_btn_primary">
                            <a href="{{url('admin/add_subadmin')}}">Add Sub Admin</a> &nbsp;&nbsp;
                        </div>
                    </div>
                </div>   
              </div> 
              <div class="col-lg-12">
                  <div class="search_list">
                      <div class="add_btn_primary">
                          <a href="{{route('subadminfile-export')}}">CSV Export</a> &nbsp;&nbsp;
                      </div>
                  </div>
              </div> 
              <div class="col-lg-12">
...
<script type="text/javascript">
    ...
    function DeleteUser(UserId)
    {
        swal({
            title: "Are you sure ??",
            text: "You will not be able to recover this subadmin!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((willDelete) => {
          if (willDelete) {
                var delurl = "{{url('admin/subadmin_delete')}}";
                $.ajax({
                    url: delurl,
                    type: "post",
                    ...
                    error: function (request, status, error) {
                        ...
                        {
                            location.href = "{{url('admin/subadmin')}}";
                        }
                    }
                });
            }
    ...
    function Status(UserId,status)
    {
        ...
        swal({
            title: "Are you sure ?",
            text: "You want to "+btn_text+" this subadmin!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((willDelete) => {
          if (willDelete) {
                var delurl = "{{url('admin/subadmin_status')}}";
                $.ajax({
                    url: delurl,
                    type: "post",
    ...
    $(".actions").click(function(e){
      ...
      if(_ids!='')
      {
        swal({
          ...
          if (willDelete) {
                var delurl = "{{url('admin/subadmin_multi_status')}}";
                $.ajax({
                    url: delurl,
                    type: "post",
